...
pragma solidity ^0.8.0;
 ...
 contract avaxTreezNftMarket is IERC721Receiver , Ownable {
    struct bid{
   ...
   // Set this address after deploy for getting fees for trades!!!!!!
    ...
    function itemsForSaleIds() public view returns(uint256[] memory){
    ...
    function removeSaleId(uint256 saleid) private {
         for(uint256 index; index < itemsForSaleList.length ; index++){
           if(itemsForSaleList[index] == saleid){
               itemsForSaleList[index] = itemsForSaleList[itemsForSaleList.length - 1];
               itemsForSaleList.pop();
    ...
    function deductFees(uint256 saleID , uint256 amount) internal returns (uint256) {
        ItemForSale storage currentSaleItem =  itemsForSale[saleID];
         ...
         if(currentPaymentMethod.fees > 0){
          ...
          feeRemitanceAddress.transfer(fees_to_deduct);
